...
class OnboardingTask(Job):  # pylint: disable=too-many-instance-attributes
    ...
    def run(self, *args, **data):
        ...
        if data["discovered_group"] is not None:
            self.discovered_ips = DiscoveredIPAddress.objects.get(discovered_group=data["discovered_group"], marked_for_onboarding=True).values_list("ip_address", flat=True)

        address_list = []
        if self.discovered_ips is not None:
            address_list = self.discovered_ips
        else:
            address_list = [address for address in data["ip_address"].replace(" ", "").split(",")]
