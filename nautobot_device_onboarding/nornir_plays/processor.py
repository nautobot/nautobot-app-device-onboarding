...
class ProcessorDONew(BaseLoggingProcessor):
    """Processor class for Device Onboarding jobs."""
    ...
    def task_instance_completed(self, task: Task, host: Host, result: MultiResult) -> None:
        ...
        if result.failed:
            ...
            self.logger.info(f"task_instance_completed Task Name: {task.name} Task Result: {result.result}", extra={"object": task.host})
