...
class TestTransformWithGitRepo(TransactionTestCase):
    ...
    def setUp(self):
        super().setUp()
        self.yaml_file_dir = f"{MOCK_DIR}/command_mappers/"
        # self.repo = GitRepository.objects.get_or_create(
        #     name="foo",
    ...
    def populate_repo(self, path, url, *args, **kwargs):
        ...
        with open(os.path.join(path, "onboarding_command_mappers", "foo_bar.yml"), "w") as fd:
            yaml.dump(
                {
                    "sync_devices": {
                        "serial": {
    ...
    def test_git_repo_was_created(self, MockGitRepo):
    ...
    def test_pull_git_repository_and_refresh_data_with_valid_data(self, mock_load_command_mappers, MockGitRepo):
        """
