...
def docker_compose(context, command, **kwargs):
    ...
    if (
        context.nautobot_device_onboarding.ephemeral_ports
        and context.nautobot_device_onboarding.compose_files == ORIGINAL_COMPOSE_FILES
    ):
        compose_file_path = os.path.join(
            context.nautobot_device_onboarding.compose_dir, "docker-compose.ephemeral-ports.yml"
        )
        compose_command_tokens.append(f' -f "{compose_file_path}"')
...
def _ensure_creds_env_file(context):
    ...
    if not os.path.exists(
        ...
        shutil.copy(
            os.path.join(
                context.nautobot_device_onboarding.compose_dir, "creds.example.env"
            ),
            os.path.join(context.nautobot_device_onboarding.compose_dir, "creds.env"),
        )
...
def pylint(context, target=None, recursive=False):
    ...
    if recursive:
        command += " --recursive=y"
    command += f" {' '.join(target) if target else 'nautobot_device_onboarding'}"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    app_dir = Path(__file__).absolute().parent / Path("nautobot_device_onboarding")
    migrations_dir = app_dir / Path("migrations")
    migrations_target_module = "nautobot_device_onboarding.migrations"
...
def generate_test_data(context, flush=False, database=None):
    """Generate test data in Nautobot for Device Onboarding."""
    command = "nautobot-server generate_nautobot_device_onboarding_test_data"
    if database:
